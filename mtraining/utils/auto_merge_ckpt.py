...
from .merge_utils import STORE_DIR, merge_ckpts, copy_configs_to_merged_dir
        ...
        for iteration in os.listdir(base_ckpt_dir):
            ...
            try:
                ...
                if is_checkpoint_complete(iter_dir, args.num_gpus):
                    ...
                    merged = merge_ckpts(
                        expr_data_dir, epoch_idx, iter_idx, args.override,
                    )
                    if not merged:
                    ...
                    copied = copy_configs_to_merged_dir(
                        expr_data_dir, epoch_idx, iter_idx,
                        args.model_id, args.override,
                    )
                    if not copied:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expr_data_dir", type=str, default=None, help="Path to the experiment data directory")
    ...
    parser.add_argument('--model_id', type=str, default='Qwen/Qwen2.5-3B', help='transformers model id')
    parser.add_argument('--use_ring_attn', action='store_true', help='use ring attention')
    ...
    if args.expr_data_dir is None and (args.gpu_set is None or args.expr_dir is None or args.expr_name is None):
        raise ValueError("Either use expr_data_dir or (gpu_set, expr_dir, and expr_name) to present the experiment data dir")
    print_args(args)

    expr_data_dir = args.expr_data_dir if args.expr_data_dir else os.path.join(
