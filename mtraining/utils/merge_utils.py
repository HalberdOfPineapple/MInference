...
def model_to_hf_config_files(model_id: str):
    if 'phi' in model_id.lower():
        return [
            "added_tokens.json", "config.json", "configuration_phi3.py", 
            "generation_config.json", "modeling_phi3.py", "generation_config.json",
            "model.safetensors.index.json", 
            "special_tokens_map.json", "tokenizer_config.json", "tokenizer.json"
        ]
    elif 'qwen' in model_id.lower():
        return [
            "config.json", "generation_config.json", "modeling_qwen2.py",
        ...
        return [
            ...
            "model.safetensors.index.json", "modeling_llama.py",
...
def recursive_equiv_state_dict(dict1, dict2):
    ...
    for key in dict1:
        if isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
            ...
            print(f"{__name__} | Values differ at key '{key}': {dict1[key]} != {dict2[key]}")
            return False
    return True
...
def merge_checkpoint(checkpoint_files: List[str], output_file: str):
    ...
    for i, file_path in enumerate(checkpoint_files):
        print(f"merge_checkpoint | Loading {i}-th checkpoint from {file_path}...", flush=True)
...
def load_ckpt_files(ckpt_dir):
    ...
    ckpt_files = [os.path.join(ckpt_dir, f) for f in os.listdir(ckpt_dir) if f.endswith('.ckpt') and not f.startswith('merged')]
...
def merge_ckpts(
    ...
    ckpt_dir = os.path.join(
        expr_data_dir, "checkpoints", f"{epoch_idx:04d}-{iter_idx:04d}",
    )
    if not os.path.exists(ckpt_dir):
        # Legacy Storage Path
        ckpt_dir = os.path.join(
            expr_data_dir, "checkpoints", "rank_0",
    ...
    if os.path.exists(merged_ckpt_save_path) and not override:
        print(f"merge_ckpts | Checkpoint already exists in {merged_ckpt_save_path} and skip merging (enabling --override to override)")
        return True

    print(f"merge_ckpts | Merging ckpt files in {ckpt_dir} to {merged_ckpt_save_path}", flush=True)
    ...
    # --------------------------------------------------------------------------
    print(f"merge_ckpts | Converting merged checkpoint to correct format...", flush=True)
...
def copy_configs_to_merged_dir(
    expr_data_dir: str, epoch_idx: int, iter_idx: int,
    model_id: str,
    override: bool = False,
):
    merged_ckpt_save_dir = os.path.join(
    ...
    if not os.path.exists(merged_ckpt_save_path):
        raise ValueError(f"Merged checkpoint path {merged_ckpt_save_path} does not exist in {expr_data_dir}. Run merge_ckpt first.")

    print('-' * 20)
    print(f"copy_configs_to_merged_dir | Copying model files to {merged_ckpt_save_dir}", flush=True)
...
def load_merged_model(
    ...
    expr_data_dir: str,
    epoch_idx: int, iter_idx: int,
    merged_ckpt_dir: Optional[str] = None,
):
    if merged_ckpt_dir is None:
        model_dir = os.path.join(expr_data_dir, "merged_ckpts", f"{epoch_idx:04d}-{iter_idx:04d}")
        model_config = AutoConfig.from_pretrained(model_dir)
        if 'mini' in model_dir: model_config.num_hidden_layers = 2
    else:
